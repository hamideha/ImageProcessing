...
class Encoder:
    ...
    def encode(self):
        ...
        Y = self.downsample(Y, 2)

        # Downsample Cb and Cr by a factor of 4
        Cb = self.downsample(Cb, 4)
        Cr = self.downsample(Cr, 4)
...
class Decoder:
    ...
    def decode(self, size):
        ...
        Y = self.bilinear(self.ycbcr[0], size)

        # Upsample Cb and Cr by a factor of 4
        Cb = self.bilinear(self.ycbcr[1], size)
        Cr = self.bilinear(self.ycbcr[2], size)
